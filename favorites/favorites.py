...
def get_favorite_car(request):
    return request.session.get(FAVORITE_PRODUCTS_KEY, [])

def get_count_of_favorite_car(request):
    return len(get_favorite_car(request))

def add_car_to_favorites(request, product_id):
    favoriteIds = get_favorite_car(request)
    if product_id not in favoriteIds:
        favoriteIds.append(product_id)
        request.session[FAVORITE_PRODUCTS_KEY] = favoriteIds
    request.session.modified = True

def remove_car_from_favorites(request, product_id):
    favoriteIds = get_favorite_car(request)
